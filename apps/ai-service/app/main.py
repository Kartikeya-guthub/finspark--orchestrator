import json
import os
from typing import Any
from pathlib import Path

from fastapi import FastAPI, HTTPException
from minio import Minio
from dotenv import load_dotenv
from psycopg import connect
from psycopg.rows import dict_row

from app.pipeline.config_generator import generate_config
from app.pipeline.extension_a_full_pipeline import full_extraction_pipeline
from app.pipeline.extension_d_nl_edit import apply_nl_instruction
from app.pipeline.extension_e_reparse import handle_brd_update
from app.pipeline.extractor import extract_requirements
from app.pipeline.mock_response import generate_mock_response
from app.pipeline.ocr import extract_plain_text, extract_structure
from app.pipeline.matcher import match_adapters
from app.pipeline.pii import redact_structured, redact_text_regex
from app.pipeline.safety import SafetyViolationError, safety_check_config

ROOT_ENV = Path(__file__).resolve().parents[3] / ".env"
load_dotenv(ROOT_ENV)

app = FastAPI(title="finspark-ai-service", version="0.1.0")

def db_url() -> str:
    database_url = os.getenv("DATABASE_URL")
    if database_url:
        return database_url
    user = os.getenv("POSTGRES_USER", "finspark")
    password = os.getenv("POSTGRES_PASSWORD", "finspark")
    host = os.getenv("POSTGRES_HOST", "127.0.0.1")
    port = os.getenv("POSTGRES_PORT", "5432")
    database = os.getenv("POSTGRES_DB", "finspark")
    return f"postgresql://{user}:{password}@{host}:{port}/{database}"


def minio_client() -> Minio:
    endpoint = os.getenv("MINIO_ENDPOINT", "127.0.0.1")
    port = os.getenv("MINIO_API_PORT", "9000")
    use_ssl = os.getenv("MINIO_USE_SSL", "false").lower() == "true"
    return Minio(
        endpoint=f"{endpoint}:{port}",
        access_key=os.getenv("MINIO_ROOT_USER", "minioadmin"),
        secret_key=os.getenv("MINIO_ROOT_PASSWORD", "minioadmin"),
        secure=use_ssl,
    )


@app.get("/health")
def health() -> dict[str, str]:
    return {"status": "ok"}


@app.post("/mock-response")
def mock_response(payload: dict[str, Any]) -> dict[str, Any]:
    adapter_schema = payload.get("adapterSchema")
    scenario = str(payload.get("scenario") or "success")

    if not isinstance(adapter_schema, dict):
        raise HTTPException(status_code=400, detail="adapterSchema must be an object")
    if scenario not in {"success", "partial_failure", "timeout", "schema_mismatch"}:
        raise HTTPException(status_code=400, detail="scenario must be success | partial_failure | timeout | schema_mismatch")

    response = generate_mock_response(adapter_schema, scenario)
    return {
        "status": "ok",
        "scenario": scenario,
        "response": response,
    }


@app.post("/extensions/full-pipeline")
async def run_full_pipeline(payload: dict[str, Any]) -> dict[str, Any]:
    document_id = str(payload.get("documentId") or "").strip()
    tenant_id = str(payload.get("tenantId") or "").strip()

    if not document_id or not tenant_id:
        raise HTTPException(status_code=400, detail="documentId_and_tenantId_required")

    try:
        result = await full_extraction_pipeline(document_id=document_id, tenant_id=tenant_id)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(status_code=500, detail=f"full_pipeline_failed: {exc}") from exc

    return {"status": "ok", **result}


@app.post("/extensions/nl-edit")
async def nl_edit_config(payload: dict[str, Any]) -> dict[str, Any]:
    config_version_id = str(payload.get("configVersionId") or "").strip()
    instruction = str(payload.get("instruction") or "").strip()
    actor = str(payload.get("actor") or "system").strip() or "system"

    if not config_version_id or not instruction:
        raise HTTPException(status_code=400, detail="configVersionId_and_instruction_required")

    try:
        result = await apply_nl_instruction(
            config_version_id=config_version_id,
            instruction=instruction,
            actor=actor,
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(status_code=500, detail=f"nl_edit_failed: {exc}") from exc

    return {"status": "ok", **result}


@app.post("/extensions/reparse-brd")
async def reparse_brd(payload: dict[str, Any]) -> dict[str, Any]:
    new_document_id = str(payload.get("newDocumentId") or "").strip()
    original_document_id = str(payload.get("originalDocumentId") or "").strip()
    tenant_id = str(payload.get("tenantId") or "").strip()

    if not new_document_id or not original_document_id or not tenant_id:
        raise HTTPException(status_code=400, detail="newDocumentId_originalDocumentId_tenantId_required")

    try:
        result = await handle_brd_update(
            new_document_id=new_document_id,
            original_document_id=original_document_id,
            tenant_id=tenant_id,
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(status_code=500, detail=f"brd_reparse_failed: {exc}") from exc

    return result


@app.post("/process-document")
async def process_document(payload: dict[str, Any]) -> dict[str, Any]:
    document_id = payload.get("documentId")
    tenant_id = payload.get("tenantId")
    object_path = payload.get("objectPath")
    filename = payload.get("filename")

    if not all([document_id, tenant_id, object_path, filename]):
        raise HTTPException(status_code=400, detail="documentId, tenantId, objectPath, filename required")

    bucket = os.getenv("MINIO_BUCKET_DOCS", "finspark-documents")
    client = minio_client()

    try:
        response = client.get_object(bucket, object_path)
        file_bytes = response.read()
        response.close()
        response.release_conn()
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(status_code=500, detail=f"failed_to_fetch_object: {exc}") from exc

    content_type = payload.get("contentType") or "application/octet-stream"

    try:
        original_text = extract_plain_text(filename, file_bytes)
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(status_code=400, detail=f"text_extraction_failed: {exc}") from exc

    redacted_text, pii_entities = redact_text_regex(original_text)
    structured = extract_structure(filename, file_bytes, str(content_type))
    structured_redacted, structured_pii_entities, chunk_count = redact_structured(structured)

    extraction = None
    config_result = None

    try:
        with connect(db_url(), row_factory=dict_row) as conn:
            with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO document_texts (document_id, tenant_id, original_text, redacted_text, pii_entities)
                VALUES (%s, %s, %s, %s, %s::jsonb)
                ON CONFLICT (document_id)
                DO UPDATE SET
                  original_text = EXCLUDED.original_text,
                  redacted_text = EXCLUDED.redacted_text,
                  pii_entities = EXCLUDED.pii_entities,
                  updated_at = now()
                """,
                (document_id, tenant_id, original_text, redacted_text, json.dumps(pii_entities)),
            )

            cur.execute(
                """
                UPDATE documents
                SET parse_status = 'processing_requirements',
                    structured_content = %s::jsonb
                WHERE id = %s AND tenant_id = %s
                """,
                (json.dumps(structured_redacted), document_id, tenant_id),
            )

            cur.execute(
                """
                INSERT INTO audit_events (tenant_id, entity_type, entity_id, action, before, after, actor)
                VALUES (%s, 'document', %s, 'pii_redaction', NULL, %s::jsonb, 'ai-service')
                """,
                (
                    tenant_id,
                    document_id,
                    json.dumps({"entities_found": pii_entities}),
                ),
            )

            cur.execute(
                """
                INSERT INTO audit_events (tenant_id, entity_type, entity_id, action, before, after, actor)
                VALUES (%s, 'document', %s, 'structure_extraction', NULL, %s::jsonb, 'ai-service')
                """,
                (
                    tenant_id,
                    document_id,
                    json.dumps(
                        {
                            "source": structured.get("source", "unknown"),
                            "sections": len(structured_redacted.get("sections", []))
                            if isinstance(structured_redacted, dict)
                            else 0,
                            "tables": len(structured_redacted.get("tables", []))
                            if isinstance(structured_redacted, dict)
                            else 0,
                            "headers": len(structured_redacted.get("headers", []))
                            if isinstance(structured_redacted, dict)
                            else 0,
                            "entities_found": [],
                        }
                    ),
                ),
            )

            cur.execute(
                """
                INSERT INTO audit_events (tenant_id, entity_type, entity_id, action, before, after, actor)
                VALUES (%s, 'document', %s, 'pii_redaction_structured', NULL, %s::jsonb, 'ai-service')
                """,
                (
                    tenant_id,
                    document_id,
                    json.dumps(
                        {
                            "entities_summary": structured_pii_entities,
                            "chunk_count": chunk_count,
                        }
                    ),
                ),
            )

            use_full_pipeline = (os.getenv("AI_USE_FULL_PIPELINE", "true").lower() == "true")
            if use_full_pipeline:
                cur.execute(
                    """
                    UPDATE documents
                    SET parse_status = 'structure_extracted'
                    WHERE id = %s AND tenant_id = %s
                    """,
                    (document_id, tenant_id),
                )
                conn.commit()

                full_result = await full_extraction_pipeline(
                    document_id=str(document_id),
                    tenant_id=str(tenant_id),
                )
                return {
                    "status": "ok" if full_result.get("safety", {}).get("safe", False) else "blocked",
                    "document_id": document_id,
                    "tenant_id": tenant_id,
                    "entities_found": pii_entities,
                    "structured_entities_found": structured_pii_entities,
                    "structure_source": structured.get("source", "unknown"),
                    "requirements_count": len(full_result.get("requirements", [])),
                    "extraction_confidence": full_result.get("extraction_confidence", 0),
                    "config_version_id": full_result.get("config_version", {}).get("id"),
                    "config_version_number": full_result.get("config_version", {}).get("version_number"),
                    "safety": full_result.get("safety"),
                    "used_stub_fallback": full_result.get("used_stub_fallback", False),
                }

            extraction = extract_requirements(
                conn=conn,
                structured_content=structured_redacted,
                document_id=document_id,
                tenant_id=tenant_id,
            )

            matched_adapters = [match_adapters(conn, requirement.model_dump()) for requirement in extraction.requirements]
            config_result = None
            safety_result = None
            if extraction.requirements:
                config_result = generate_config(
                    conn=conn,
                    requirements=[requirement.model_dump() for requirement in extraction.requirements],
                    matched_adapters=matched_adapters,
                    tenant_id=tenant_id,
                    document_id=document_id,
                    structured_content=structured_redacted,
                )
                safety_result = safety_check_config(config_result["config_json"])
                if not safety_result.get("safe", False):
                    raise SafetyViolationError(list(safety_result.get("violations", [])))

            with conn.cursor() as cur:
                parse_status = "requirements_extracted"
                if config_result:
                    parse_status = "config_generated"

                cur.execute(
                    """
                    UPDATE documents
                    SET parse_status = %s
                    WHERE id = %s AND tenant_id = %s
                    """,
                    (
                        parse_status,
                        document_id,
                        tenant_id,
                    ),
                )

                cur.execute(
                    """
                    INSERT INTO audit_events (tenant_id, entity_type, entity_id, action, before, after, actor)
                    VALUES (%s, 'document', %s, 'requirement_extraction', NULL, %s::jsonb, 'ai-service')
                    """,
                    (
                        tenant_id,
                        document_id,
                        json.dumps(
                            {
                                "requirements_count": len(extraction.requirements),
                                "extraction_confidence": extraction.extraction_confidence,
                                "ambiguous_requirements": extraction.ambiguous_requirements,
                                "missing_information": extraction.missing_information,
                            }
                        ),
                    ),
                )

                if config_result:
                    cur.execute(
                        """
                        INSERT INTO audit_events (tenant_id, entity_type, entity_id, action, before, after, actor)
                        VALUES (%s, 'tenant_config', %s, 'config_generation', NULL, %s::jsonb, 'ai-service')
                        """,
                        (
                            tenant_id,
                            config_result["version_id"],
                            json.dumps(
                                {
                                    "tenant_config_id": str(config_result["tenant_config_id"]),
                                    "version_number": config_result["version_number"],
                                    "requirements_count": len(extraction.requirements),
                                    "match_results": config_result["match_results"],
                                },
                                default=str,
                            ),
                        ),
                    )

                    if safety_result and safety_result.get("safe", False):
                        cur.execute(
                            """
                            UPDATE tenant_config_versions
                            SET status = 'pending_review'
                            WHERE id = %s
                            """,
                            (config_result["version_id"],),
                        )
                        cur.execute(
                            """
                            INSERT INTO audit_events (tenant_id, entity_type, entity_id, action, before, after, actor)
                            VALUES (%s, 'tenant_config', %s, 'safety_check_passed', NULL, %s::jsonb, 'ai-service')
                            """,
                            (
                                tenant_id,
                                config_result["version_id"],
                                json.dumps(safety_result),
                            ),
                        )

            conn.commit()

    except SafetyViolationError as exc:
        with connect(db_url(), row_factory=dict_row) as safety_conn:
            with safety_conn.cursor() as cur:
                if config_result:
                    cur.execute(
                        """
                        UPDATE tenant_config_versions
                        SET status = 'blocked'
                        WHERE id = %s
                        """,
                        (config_result["version_id"],),
                    )
                cur.execute(
                    """
                    UPDATE documents
                    SET parse_status = 'config_blocked'
                    WHERE id = %s AND tenant_id = %s
                    """,
                    (document_id, tenant_id),
                )
                if config_result:
                    cur.execute(
                        """
                        INSERT INTO audit_events (tenant_id, entity_type, entity_id, action, before, after, actor)
                        VALUES (%s, 'tenant_config', %s, 'safety_check_failed', NULL, %s::jsonb, 'ai-service')
                        """,
                        (
                            tenant_id,
                            config_result["version_id"],
                            json.dumps(
                                {
                                    "violations": exc.violations,
                                    "config_blocked": True,
                                }
                            ),
                        ),
                    )
            safety_conn.commit()

        return {
            "status": "blocked",
            "document_id": document_id,
            "tenant_id": tenant_id,
            "entities_found": pii_entities,
            "structured_entities_found": structured_pii_entities,
            "structure_source": structured.get("source", "unknown"),
            "requirements_count": len(extraction.requirements),
            "extraction_confidence": extraction.extraction_confidence,
            "config_version_id": None if config_result is None else config_result["version_id"],
            "config_version_number": None if config_result is None else config_result["version_number"],
            "safety": {
                "safe": False,
                "violations": exc.violations,
                "warnings": [],
                "recommendation": "reject",
            },
        }

    return {
        "status": "ok",
        "document_id": document_id,
        "tenant_id": tenant_id,
        "entities_found": pii_entities,
        "structured_entities_found": structured_pii_entities,
        "structure_source": structured.get("source", "unknown"),
        "requirements_count": len(extraction.requirements),
        "extraction_confidence": extraction.extraction_confidence,
        "config_version_id": None if config_result is None else config_result["version_id"],
        "config_version_number": None if config_result is None else config_result["version_number"],
        "safety": safety_result,
    }
